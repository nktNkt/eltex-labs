...
int main() {
	...
	int fd;
	fd = open("contacts.txt", O_RDONLY);
	if (fd == -1) printf("File contacts.txt not found\n");
	else {
	...
	int fdsave;
	fdsave = creat("contacts.txt", 0666);
	if (fdsave == -1) printf("Error on create contacts.txt");
	else {
		struct Item* tmp = contacts;
		while(tmp) {
			p = tmp->value;
			write(fdsave, &p, sizeof(p));
			tmp = tmp->next;
		}
		close(fdsave);
	}
