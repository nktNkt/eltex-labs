...
int main(int argc, char* argv[]) {
	...
	struct sembuf lockFileParent = {0, -PNUM, 0};
	struct sembuf unlockFileParent = {0, PNUM, 0};
	...
	case 0:
		...
		for (int i = 0, n = -999999; i < nums; i++) {
			...
			semop(sem, &lockFile, 1);		// 8. Забрать доступ к файлу (чтение)
	...
	default:
		...
		while(read(pipes[0], &buf, sizeof(buf)) > 0) {
			semop(sem, &lockFileParent, 1);			// 4. Забрать доступ к файлу (запись)
			fd = open("numbers.txt", O_WRONLY | O_APPEND);
			write(fd, &buf, sizeof(buf));			// 5. Запись числа в файл
			close(fd);
			semop(sem, &unlockFileParent, 1);		// 6. Освободить доступ к файлу
